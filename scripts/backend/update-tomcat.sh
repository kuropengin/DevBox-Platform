...
# 使い方: sudo bash scripts/backend/update-tomcat.sh [メジャーバージョン...]
#         例: sudo bash scripts/backend/update-tomcat.sh          # 9 と 11 の両方
#             sudo bash scripts/backend/update-tomcat.sh 9         # 9 のみ
#
...
[[ $EUID -ne 0 ]] && die "rootで実行してください: sudo bash scripts/backend/update-tomcat.sh [メジャーバージョン...]"
